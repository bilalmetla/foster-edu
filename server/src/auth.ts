...
export const BEARER_STRATEGY_NAME = 'bearer';
...
export function secured(
  ...
  roles: string[] = [],
  strategy: string = 'jwt', //default is jwt
  options?: object,
) {
...
export const JWT_SECRET = 'SSS@123456789';
...
export class MyAuthAuthenticationStrategyProvider implements Provider<AuthenticationStrategy | undefined> {
  ...
  async verifyToken(
    ...
    try {
      logger.silly(`auth payload' ${JSON.stringify(payload)} `);
      const {username} = payload;
      const user = await this.userRepository.findOne({"where":{username:username}}); //await this.userRepository.findById(username);
      if (!user) done(null, false);

      await this.verifyRoles(username);

      if(user)
        done(null, { phone: user.username, [securityId]: username});
